close all;
clear all;
img1 = imread('../dataset/notredame13/01.png');
img2 = imread('../dataset/notredame13/02.png');
...
for i=1:K
    frames1{i} = extractFrames(MSERfeat1{i});
    frames2{i} = extractFrames(MSERfeat2{2});
end
%Get Sift Features(Probem: Matlab Crashing for this)
% for i=1:K 
%     [siftfeat1{i},vpts1{i}] = vl_sift(im2single(uint8(mat_eig1{i})),'frames',frames1{i}','orientation');
%     [siftfeat2{i},vpts2{i}] = vl_sift(im2single(uint8(mat_eig2{i})),'frames',frames2{i}','orientation');
% end
%Method For SIFT features
% for i=1:K
...
for i=1:K
    figure;showMatchedFeatures(im1,im2,matched_pts1{i},matched_pts2{i},'montage');
end
for i=1:K
    f = figure;
    colormap('jet');
